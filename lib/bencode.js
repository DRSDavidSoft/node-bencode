...
Bencode.decode = function(buffer) {
  return Bencode.parse(Bencode.tokenize(buffer));
};
...
Bencode.parse = function(tokens) {
  ...
  while (pos < tokens.length) {
    switch (state) {
      ...
      case LIST:
        pos++;
        var list = Bencode.parse(tokens.splice(pos, tokens.indexOf('e')));
        output.push(list);
        state = ACCEPT;
        break;
      ...
      case DICT:
        pos++;
        var entries = Bencode.parse(tokens.splice(pos, tokens.indexOf('e')));
        var dict = {};
